...
/* ─── Página ──────────────────────────────────────────────── */
export default function AdminGeograficoPage() {
  ...
  useEffect(() => {
    fetch("/api/admin/geografico")
      .then(r => {
        ...
        if (r.status === 403) { setErro("Acesso restrito a administradores."); return null }
        return r.json()
      })
  ...
  if (erro || !data) return (
    <div className="min-h-screen flex items-center justify-center">
      <div className="text-center">
        <p className="mb-4" style={{ color: "#dc2626" }}>{erro || "Erro"}</p>
        <button onClick={() => router.push("/admin")} className="btn-green">← Voltar ao Admin</button>
      </div>
    </div>
  )
    ...
    <main className="min-h-screen" style={{ background: "#f9fafb" }}>
      ...
      <header style={{ background: "#006635" }} className="px-6 py-3">
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div className="h-5 w-px opacity-40" style={{ background: "#fff" }} />
            <span className="text-white font-semibold text-sm">Análise Geográfica</span>
          </div>
      ...
      <div className="max-w-7xl mx-auto px-6 py-8">
        ...
        <div className="mb-6">
          ...
          <h1 className="text-2xl font-black mb-1">Análise Geográfica</h1>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
          ...
          {/* Estado pior */}
          <div className="rounded-xl border bg-white px-5 py-4 overflow-hidden relative"
          ...
          {/* Estado melhor */}
          <div className="rounded-xl border bg-white px-5 py-4 overflow-hidden"
          ...
          <div className="rounded-xl border bg-white px-5 py-4"
            ...
            {kpis.cidadeMaisCritica ? (
              <>
                ...
                <div className="text-xs mt-1" style={{ color: "#9f9f9f" }}>{kpis.cidadeMaisCritica.totalEmpresas} empresa{kpis.cidadeMaisCritica.totalEmpresas !== 1 ? "s" : ""}</div>
              </>
            ) : <div style={{ color: "#c8c8c8" }}>—</div>}
          </div>
        ...
        {/* Tabs */}
        <div className="flex gap-1 mb-6 p-1 rounded-xl w-fit" style={{ background: "#f1f1f1" }}>
          {([
        ...
        {tab === "estados" && (
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                        ...
                        <div className="flex items-center gap-3">
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <div className="flex gap-3 mt-1 text-xs" style={{ color: "#9f9f9f" }}>
                              ...
                              {e.empresasSemRelatorio > 0 && (
                                <span style={{ color: "#d97706" }}>⚠ {e.empresasSemRelatorio} sem relatório</span>
                              )}
                            </div>
                          </div>
            ...
            {/* Painel lateral — dimensões do estado selecionado */}
            <div>
              {estadoSelected ? (
          ...
          <div className="space-y-4">
                    ...
                    <div className="px-5 py-4">
                      ...
                      <div className="grid grid-cols-3 gap-2 mb-3">
                        <div className="text-center p-2 rounded-lg" style={{ background: "#f9fafb" }}>
                          <div className="text-lg font-black" style={{ color: "#1a1a1a" }}>{r.totalEmpresas}</div>
                          <div className="text-xs" style={{ color: "#9f9f9f" }}>Total</div>
                        </div>
                        <div className="text-center p-2 rounded-lg" style={{ background: "#f9fafb" }}>
                          <div className="text-lg font-black" style={{ color: "#006635" }}>{r.empresasComRelatorio}</div>
                          <div className="text-xs" style={{ color: "#9f9f9f" }}>Avaliadas</div>
                        </div>
                        <div className="text-center p-2 rounded-lg" style={{ background: "#f9fafb" }}>
                          <div className="text-lg font-black"
                            style={{ color: r.empresasSemRelatorio > 0 ? "#d97706" : "#9f9f9f" }}>
                            {r.empresasSemRelatorio}
                          </div>
                          <div className="text-xs" style={{ color: "#9f9f9f" }}>Pendentes</div>
                        </div>
                      </div>
            ...
            {porRegiao.length > 1 && (
              <div className="rounded-xl border bg-white overflow-hidden mt-6"
                style={{ borderColor: "#e8e8e8" }}>
                <div className="px-5 py-4 border-b font-bold text-sm" style={{ borderColor: "#f1f1f1" }}>
                ...
                <table className="w-full text-sm">
                  ...
                  <tbody>
                      ...
                      <tr key={`${c.cidade}-${c.uf}`}
                        ...
                        <td className="px-4 py-3">
                          <span className="font-bold text-xs px-2 py-0.5 rounded"
                            style={{ background: "#f1f1f1", color: "#505050" }}>{c.uf}</span>
                        </td>
                        ...
                        <td className="px-4 py-3">{c.categoria ? <Badge cat={c.categoria} /> : <span style={{ color: "#9f9f9f", fontSize: "0.75rem" }}>Sem relatório</span>}</td>
                        <td className="px-4 py-3 min-w-28">
                          <DistBar pctG={c.pctGrave} pctC={c.pctCritico} pctS={c.pctSatisfatorio} total={c.empresasComRelatorio} />
                          <div className="text-xs mt-0.5" style={{ color: "#c8c8c8" }}>
                    ...
                    {cidadesFiltradas.length === 0 && (
                      <tr>
                        <td colSpan={9} className="px-4 py-12 text-center" style={{ color: "#9f9f9f" }}>
